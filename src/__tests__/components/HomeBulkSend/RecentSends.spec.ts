...
const stubs = {
  MissingRecentSends: {
    ...
    template: '<div class="missing-recent-sends-stub" @click="$emit(\'start-new-send\')">Missing Recent Sends</div>'
  },
  UnnnicInput: {
    props: ['placeholder', 'modelValue', 'iconLeft'],
    emits: ['update:modelValue'],
    template: '<input class="unnnic-input-stub" :placeholder="placeholder" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" />'
  },
