...
const stubs = {
  UnnnicToolTip: {
    props: ['side'],
    template:
      '<div class="unnnic-tooltip-stub" :data-side="side"><slot /></div>',
  },
  UnnnicIcon: { template: '<div class="unnnic-icon-stub"></div>' },
  UnnnicDropdown: {
    template:
      '<div class="unnnic-dropdown-stub"><slot name="trigger" /><slot /></div>',
  },
  UnnnicDropdownItem: {
    emits: ['click'],
    template:
...
const checkCell = (cell: DOMWrapper<Element>, label: string, value: string) => {
...
describe('MetricsTable.vue', () => {
  it('should render rows and cells correctly based on maxColumns', () => {
    ...
    const firstRowCells = rows[0].findAll('.metrics-table__cell');
    expect(firstRowCells).toHaveLength(2);
    checkCell(firstRowCells[0], 'Metric 1', '100');
    checkCell(firstRowCells[1], 'Metric 2', '200');

    const secondRowCells = rows[1].findAll('.metrics-table__cell');
  ...
  it('should display a tooltip when hint is provided', () => {
    ...
    const cells = wrapper.findAll('.metrics-table__cell');
